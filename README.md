...
Для запуска скрипта необходимо создать файл .env в директории проекта и прописать в нем данные для подключения к бд (пример данных находится в файле .env.example). 

Пример запуска из консоли: cd C:\Users\ME\PycharmProjects\Inochkin_A_S-PTMK_test\; затем: python main.py 6 (или номер другого режима из задания). Сначала в консоли нужно прописать "cd" и путь до файла, т.к. в коде указывается путь к текущей директории (Path.cwd()). Если запускать без "cd", то меняется текущая директория и выпадает ошибка, так как python не может найти нужный мне файл. Пока пофиксить не смог.
